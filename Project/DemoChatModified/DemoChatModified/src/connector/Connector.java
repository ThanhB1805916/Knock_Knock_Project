...
public class Connector {
	...
	private void addMessageAction(Request request) {
		...
		Platform.runLater(()->Controller.getInstance().inputMessage(message));
	}
	...
	private void acceptFriendAction(Person person) {
		Controller.getInstance().inputAcceptFriend(person);
	}
	...
	private void removeFriendAction(Request request) {
		...
		Platform.runLater(()->Controller.getInstance().inputKickFriend());
	}

	private void confirmFriendAction(Request request) {
		ConfirmFriendModel confirm = (ConfirmFriendModel) request.getContent();
		if(confirm.getSender().getId()==Controller.getInstance().getUserId()){
			if(confirm.getIsFriend()==true)
				acceptFriendAction(confirm.getFriend());
			else delineFriendAcction(confirm.getFriend());
		}
		else if(Controller.getInstance().getPersonById(confirm.getSender().getId())==null)
				Platform.runLater(()->Controller.getInstance().inputRequestConfirmFriend(confirm));
	}
	...
	private void addMemberRoom(Request request) {
		...
		if(Controller.getInstance().getRoomById(roomPersonModel.getRoom().getId())!=null)
			Controller.getInstance().inputAddMemberToARoom(roomPersonModel.getPerson(), roomPersonModel.getRoom().getId());
		else
			Platform.runLater(()->Controller.getInstance().inputToAddToRoom(roomPersonModel.getRoom()));
	}

	private void updateRoom(Request request) {
		Room room = (Room) request.getContent();
		Platform.runLater(()->Controller.getInstance().inputUpdateRoom(room));
	}
