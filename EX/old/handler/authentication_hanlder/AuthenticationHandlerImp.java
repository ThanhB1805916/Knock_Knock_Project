...
public class AuthenticationHandlerImp extends Handler implements AuthenticationHandler {
	...
	public AuthenticationHandlerImp(PersonDAO dao) {
		this.dao = dao;
	}
	...
	public void handleRequest(Client client, Request request) {
		if (request.isValid()) {
			...
			case LOGIN:
				responseCommandType = new Request(Name.LOGIN, login(client, (LoginModel) request.getContent()));
				break;
	...
	@Override
	public boolean login(Client client, LoginModel model) {
	...
	@Override
	public boolean logout(Client client, Person person) {
		// Keep connection
		// Remove person
